...
function ProfileSettings({ profile, onSave }: { profile: LearningProfile | null; onSave: (p: Partial<LearningProfile>) => Promise<void> }) {
  ...
  useEffect(() => {
    // Sync all form fields from the profile prop whenever the profile changes
    // (e.g. when it loads asynchronously after mount). Previously only two of
    // the five fields were synced here, which left persona/difficulty/dailyGoal
    // stuck at their initial defaults if the profile arrived after mount.
