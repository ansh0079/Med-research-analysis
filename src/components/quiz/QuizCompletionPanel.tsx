...
export function QuizCompletionPanel({
  ...
  onNewQuestions: () => void;
  onBackToRun: (runId: number) => void;
  onBackToPapers: () => void;
  onSignIn: () => void;
  onContinueStudyRun: (run: StudyRun) => void;
  ...
  const missedPapers = buildMissedPapers(quiz, resolveSourceArticle);
    ...
    <div className="space-y-4">
      <div className="rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 p-8 text-center shadow-sm">
        ...
        <div className="flex flex-wrap justify-center gap-3">
          <button type="button" onClick={onNewQuestions}
          ...
          {activeStudyRunId ? (
            ...
            <button type="button" onClick={onBackToPapers}
      ...
      {missedPapers.length > 0 && <MissedPapersPanel missedPapers={missedPapers} missedCount={quiz.questions.length - quiz.score} />}
      ...
      {activeStudyRunId && studyRun && studyOutline && studyOutline.nodes.length > 0 && (
        <div className="rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 p-6 shadow-sm">
          ...
          <StudyRunPanel
            ...
            gapReportMode
            onContinue={() => onContinueStudyRun(studyRun)}
          />
        </div>
      )}
        ...
        <div className="rounded-2xl border border-amber-200 dark:border-amber-800/50 bg-amber-50 dark:bg-amber-950/30 p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-amber-600 dark:text-amber-400 mt-0.5">
              ...
              <button type="button" onClick={() => onViewRun(activeStudyRunId)}
      ...
      <div className="rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 p-6 shadow-sm">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <select
            ...
            onChange={(event) => onReflectionKindChange(event.target.value as ReflectionKind)}
...
function buildMissedPapers(
  quiz: QuizState,
  resolveSourceArticle: (question: QuizQuestion) => QuizArticle | null,
): MissedPaper[] {
  const missedPapers = new Map<string, MissedPaper>();

  quiz.questions.forEach((question) => {
    const wrong = (quiz.answers[question.id] || '').toLowerCase() !== question.correctAnswer.toLowerCase();
    if (!wrong) return;

    const source = resolveSourceArticle(question);
    if (!source) return;

    const key = source.uid || source.title;
    const existing = missedPapers.get(key);
    if (existing) {
      existing.missCount += 1;
      if (question.questionType && !existing.questionTypes.includes(question.questionType)) {
        existing.questionTypes.push(question.questionType);
      }
      return;
    }

    missedPapers.set(key, {
      article: source,
      missCount: 1,
      questionTypes: question.questionType ? [question.questionType] : [],
    });
  });

  return [...missedPapers.values()].sort((a, b) => b.missCount - a.missCount);
}

function MissedPapersPanel({
  missedPapers,
  missedCount,
}: {
  ...
  return (
    <div className="rounded-2xl bg-white dark:bg-slate-800 border border-red-100 dark:border-red-900/30 p-6 shadow-sm">
      ...
      <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
        You missed {missedCount} question{missedCount === 1 ? '' : 's'} grounded in the following evidence.
      </p>
      <div className="space-y-3">
        {missedPapers.map(({ article, missCount, questionTypes }) => {
          const year = article.pubdate?.slice(0, 4) ?? null;
          const journal = article.source ?? article.journal ?? null;
          const paperUrl = article.pmid ? `https://pubmed.ncbi.nlm.nih.gov/${article.pmid}/` : article.doi ? `https://doi.org/${article.doi}` : null;
          return (
