...
function CaseErrorAlert({ error, usageLabel }: { error: string; usageLabel: string }) {
  ...
  if (error.startsWith('USAGE_LIMITED:') || error.startsWith('UPGRADE_REQUIRED:')) {
    const info = error.startsWith('USAGE_LIMITED:') ? parseUsageLimitError(error) : null;
    return (
      <div className="flex items-center gap-2 px-3 py-2.5 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50 rounded-xl">
        <i className="fas fa-chart-line text-amber-500 text-xs shrink-0" />
        <p className="text-sm text-amber-700 dark:text-amber-300">
          {info ? formatUsageLimitMessage(info) : usageLabel}{' '}
...
export function CaseInputCard({
  ...
  structuredCase: StructuredCaseFields;
  caseText: string;
  charsLeft: number;
  isOverLimit: boolean;
  loading: boolean;
  ...
  error: string | null;
  hasPayload: boolean;
  onUpdateField: (field: keyof StructuredCaseFields, value: string) => void;
  ...
  return (
    <div className="neo-card rounded-2xl p-5 space-y-3">
      ...
      <div className="flex items-center justify-between mb-1">
        ...
        <button type="button" onClick={() => onUseExample(EXAMPLE_CASE)}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {caseText && (
            <button type="button" onClick={onClear} className="text-xs text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
              Clear
            </button>
          )}
          <Button variant="secondary" size="sm" onClick={onRunCaseToEvidence}
            disabled={!hasPayload || isOverLimit || loading} isLoading={evidenceLoading}
          ...
          <Button variant="gradient" size="sm" onClick={onRunAnalysis}
            disabled={!hasPayload || isOverLimit || evidenceLoading} isLoading={loading}
      ...
      {evidenceError && <CaseErrorAlert error={evidenceError} usageLabel="Usage limit reached." />}
      {error && <CaseErrorAlert error={error} usageLabel="Monthly limit reached." />}
    </div>
  );
}
