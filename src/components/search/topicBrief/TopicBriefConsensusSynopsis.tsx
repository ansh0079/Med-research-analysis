...
export const TopicBriefConsensusSynopsis: React.FC<Props> = ({ consensusSynopsis }) => {
  ...
  const abstractOnlyEvidence = Number(consensusSynopsis.abstractPaperCount || 0) > 0
    && consensusSynopsis.freePaperCount === 0;
  const noGuidelineSupport = !consensusSynopsis.guidelineAlignment?.summary
    && !['aligned', 'partially_aligned', 'guideline_supported'].includes(
      String(consensusSynopsis.guidelineAlignment?.status || '')
    );
  const showHardBanner = (coveragePct === 0 || abstractOnlyEvidence) && noGuidelineSupport;
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {showHardBanner && (
            <div
              role="alert"
              className="mt-3 rounded-lg border-2 border-rose-500/70 bg-rose-50 px-3 py-2.5 dark:border-rose-600/60 dark:bg-rose-950/30"
            >
              ...
              <p className="mt-1 text-[11px] leading-relaxed text-rose-900/90 dark:text-rose-100/90">
                This consensus brief used abstracts/metadata only and has no guideline alignment. Prefer primary full-text sources or a guideline-backed brief before changing practice.
              </p>
            </div>
          )}
