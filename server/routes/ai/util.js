...
/**
 * Registers /api/ai/providers, /api/ai/refine, /api/ai/health.
 */
function registerAiUtilRoutes(app, {
    db,
    serverConfig,
    ai,
    logger,
    ...
    app.post('/api/ai/refine', requireAuthJwt, rateLimit(30, 60), async (req, res) => {
        ...
        try {
            ...
            if (topic) {
                const tm = await db.getUserTopicMemory(req.user.id, topic).catch((err) => {
                    logger.warn({ err }, 'getUserTopicMemory failed');
                    return null;
                });
                if (tm && tm.memoryTier === 'sparse') {
                    await db.recordUserTopicSavedArticleSignal?.(req.user.id, topic, 'refine-feedback')
                        .catch((err) => { logger.warn({ err }, 'recordUserTopicSavedArticleSignal failed'); });
                }
            }
