...
const { createAiService, AI_DISCLAIMER } = require('../../services/aiService');
...
function registerJournalClubRoutes(app, deps) {
    const {
        db, serverConfig, fetchImpl,
        requireJson, requireAuthJwt, requireVerifiedEmail, requirePaidFeature,
        aiUserLimit, validateBody, schemas,
    } = deps;
    ...
    app.post('/api/ai/journal-club', limitBodySize(2 * 1024 * 1024), requireJson, requireAuthJwt, requireVerifiedEmail, requirePaidFeature('journalClub'), aiUserLimit(8, 60), validateBody(schemas.journalClub), async (req, res) => {
        ...
        const topArticles = [...articles]
            ...
            .slice(0, 8);
        try {
            // Creates its own service instance (intentional: no LLM usage logger for journal club).
